...
const Menu = ({ id, data }: IMenu) => {
    ...
    return (
        <section className='w-fit min-h-screen min-w-fit bg-[#E9E9E9] flex flex-col justify-between'>
            <nav className='flex flex-col text-xl'>
                { MenuList.map( (item, index) => {
                    if(data?.permissionId && item.permission <= data?.permissionId) {
                        ...
                        className={ `flex items-center justify-center gap-3 duration-200 px-4 py-4 w-full group/svg ${ (item.id === id) ? `cursor-default text-[#BC9CCC] fill-white bg-gray-800` : `fill-[#2B2B2B] cursor-pointer text-[#B0B0B0] hover:bg-[#151515] hover:fill-white` }` }>
                        { item.svg }
                    </a>
                    }
                } ) }
