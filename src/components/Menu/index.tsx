...
const Menu = ({ id }: IMenu) => {
    ...
    return (
        <section className='max-w-fit min-h-screen w-full min-w-fit bg-black flex flex-col justify-between'>
            <nav className='flex flex-col text-xl w-full'>
                { MenuList.map( (item, index) => (
                    <a key={ index } onClick={() => navigate( item.href ) } 
            ...
            <nav className='flex flex-col text-xl w-full'>
                 ...
                 } } className='flex items-center gap-3 duration-200 cursor-pointer px-3 py-4 text-[#B0B0B0] w-full hover:bg-[#151515]'>
