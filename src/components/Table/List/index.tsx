    ...
    return (
        <div className='absolute top-0 left-0 w-full min-h-screen flex justify-center items-center' style={ {
            ...
            <div className='w-full max-w-[500px] h-fit bg-white rounded-md p-4 m-2 flex flex-col gap-2'>
                ...
                <hr />
                { (loading) ? <LoadingPage /> : <div className='flex flex-col gap-2 max-h-[280px] overflow-scroll'>
                    
                    { data?.length !== 0 ? <table className='h-[20px] overflow-scroll'>
                        <tbody>
                            <tr className='text-sm text-gray-600'>
                                <th className='text-left font-medium pb-3'>ФИО</th>
                            ...
                            </tr>
                            {
                                data && data.map( (user, index) => {
                                    return (
                                        <tr key={index}>
                                            <td className='text-left text-sm pr-2 flex row items-center gap-2'>
                                                <a title={ 'Tel: +' + user.number } href={ `tel:+${ user.number }` } className='text-base text-white w-inherit bg-black px-3 py-3 rounded-md flex items-center justify-center gap-2 hover:bg-gray-800'>
                                                ...
                                                <p className='flex flex-col'>
                                                    ...
                                                    <span className='text-[12px] text-gray-500 mt-[-0.2em]'>+{ user.number }</span>
                                                </p>
                                            </td>
                                            ...
                                            <td className='text-right text-sm px-2'>{ 
                                                (user.status === '1') ? 'Прочитал(-а)' :
                                                (user.status === '0') ? 'Отправлено' : 'Не отправлено'
                                            }</td>
                                        </tr>
                                    )
                                } )
